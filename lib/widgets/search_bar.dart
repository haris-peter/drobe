...
class _SearchBarState extends State<SearchBar> {
  late List _listToSearch;
  late TextEditingController _searchController;
  late String _searchText;
  late List<int> _searchedList;
  late void Function(List<int> searchList) _updateParent;
  ...
  void initState() {
    ...
    _searchText = '';
    _searchedList = [];
    _updateParent = widget.updateParent ?? (List<int> searchList) {};
  ...
  List<int> _searchList() {
    if (_searchText.isEmpty) {
      ...
      for (int i = 0; i < _listToSearch.length; i++) {
        if (_listToSearch[i].toString().contains(_searchText)) {
          _searchedList.add(i);
        }
      }
    }
  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: const EdgeInsets.fromLTRB(28.0, 20.0, 28.0, 10.0),
      child: Material(
